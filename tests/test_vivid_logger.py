...
class TestVividLogger:
    test_messages = ["This is a message", "SEVERE WARNING", "GENERIC ERROR"]
    ...
    def test_write(self, capfd):
        ...
        assert out == f"[\033[0;33m WARNING \033[0;0m] {self.test_messages[0]}\n"

        logger.msg_b()
        out, err = capfd.readouterr()
        assert out == f"[\033[1;31m CRITICAL \033[0;0m] {self.test_messages[1]}\n"

        logger.msg_c()
        out, err = capfd.readouterr()
        assert out == f"[\033[0;31m ERROR \033[0;0m] {self.test_messages[2]}\n"
    ...
    def test_print_history(self, capfd):
        ...
        out = out.split("\n")
        msgs = [
            f"[\033[0;33m WARNING \033[0;0m] {self.test_messages[0]}",
            f"[\033[1;31m CRITICAL \033[0;0m] {self.test_messages[1]}",
            f"[\033[0;31m ERROR \033[0;0m] {self.test_messages[2]}",
        ]
        for i in range(0, 3):
