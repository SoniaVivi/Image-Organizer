...
class TagController:
    ...
    def remove(self, id, tag_name):
        ...
        image_tag_id = self.db.get_id("ImageTag", {"image_id": id, "tag_id": tag_id})
    ...
    def find(self, tags):
        ...
        return tuple(
            map(lambda image: dict(zip(self.image_table_columns, image)), images)
        )
    ...
    def _all_from_image_id(self, id):
        ...
        if not tags:
            return ()
        sql = self.db.sql_from_lists("id", [tag["tag_id"] for tag in tags], "OR")
        return tuple(
            [
                x[0]
                for x in self.db.execute(f"SELECT name FROM Tag WHERE {sql}").fetchall()
            ]
        )
