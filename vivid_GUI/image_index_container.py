...
class ImageIndexContainer(ScrollView):
    ...
    def set_child(self, name="image_index"):
        ...
        if name == "image_index":
            ...
            self.current_child = (TagList(search=self.search_from_tag_list), "tag_list")
    ...
    def get_pos(self, obj, e, *args):
        if (
            self.scroll_start <= self.scroll_y
            or not self.scrolling
            or e.button != "scrollup"
        ):
            self.scrolling = False
        elif 800 >= self.to_local(*e.pos)[1] and self.current_child[1] == "image_index":
            self.current_child[0].get_images()
        self.scrolling = False
    ...
    def key_handler(self, *args):
        if self.active_widget != "workspace" or self.current_child[1] != "image_index":
            return
        # Page down key
        if 281 in args:
