...
class ToolbarModal(ModalView):
    ...
    def __init__(self, **kwargs):
        ...
        self.file_chooser = FileChooserIconView(dirselect=True, multiselect=True)
        self.file_options = FileOptions(
            options=["Only Unique", "Scramble Names", "Use Hash As Name"]
        )
        ...
        container.add_widget(
            Button(
                ...
                size_hint_max_x=100,
                size_hint_max_y=32,
                on_press=lambda x: self.add_file(),
            )
        )
...
class FileOptions(GridLayout):
    ...
    def get_selected(self):
        ...
        for child in self.children:
            selected[
                re.sub(r"\s", "_", child.children[-1].text.lower())
            ] = child.children[-2].active
        return selected
